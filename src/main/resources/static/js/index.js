...
/*
	...
	2. 요청주소 : /api/v1/todolist/list/${type}?page=페이지 번호&contentCount=글의 갯수
	...
	3. AJAX 요청을 활용
		...
		- error : 함수 -> 요청 실패 시. 400 ~
				- 400 에러의 경우
					- 매개변수 타입이 잘 못된 경우
					- 요청 리소스가 잘 못된 경우(Not Found)
	...
	const todoContentText = todoContent.querySelector(".todo-content-text");
		...
		todoContentInput.onkeyup = () => {
			if(window.event.keyCode == 13) {
				eventFlag = false;
				let responseFlag = updateTodoContent(todoCode, todo)
			}
		} 
}
